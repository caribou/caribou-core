...
(defn insert
  ...
  [table values]
  (out :db (clause "insert into %1 values %2" [(name table) (value-map values)]))
...
(defn update
  ...
  [table where values]
  (out :db (str "update " table " " where " set " values))
  (if (not (empty? values))
    (try
...
;;         t (str q w)]
;;     (sql/do-commands (out :db t))))

(defn delete
  "delete out of the given table according to the supplied where clause"
  [table & where]
  (out :db (clause "delete from %1 values %2" [(name table) (clause (first where) (rest where))]))
...
(defn create-table
  ...
  [table & fields]
  (out :db (clause "create table %1 %2" [(name table) fields]))
  (try
...
(defn rename-table
  ...
  [table new-name]
  (let [rename (out :db (clause "alter table %1 rename to %2" [(name table) (name new-name)]))]
    (try
...
(defn drop-table
  ...
  [table]
  (let [drop (out :db (clause "drop table %1 cascade" [(name table)]))]
    (try
...
(defn create-index
  ...
  (try
    (sql/do-commands
     (out :db (clause "create index %1_%2_index on %1 (%2)" (map #(zap (name %)) [table column]))))
    (catch Exception e (render-exception e))))
...
(defn set-default
  ...
  (let [value (sqlize default)]
    (sql/do-commands
     (out :db (clause "alter table %1 alter column %2 set default %3" [(zap table) (zap column) value])))))
