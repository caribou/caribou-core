      ...
      (let [[included
        ...
        (when-not included-field
          (throw (new Exception (str "field '"
                                     ...
                                     (:slug model)))))
        (and new-opts
             (validate included-field new-opts models))
        true))))
...
(defn for-associated-id
  [this id-field opts models]
  (let [slug (-> this :row id-field models :slug)]
    (beams slug opts models)))
...
(defn for-assoc
  [this opts models]
  (for-associated-id this :target_id opts models))
